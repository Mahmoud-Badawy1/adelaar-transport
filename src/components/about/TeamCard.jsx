...
function TeamCard({ id, selectedCard, handleCardClick }) {
    ...
    return (
        <div
            className={`bg-gray1 h-80 w-full sm:w-60 md:w-72 flex ${isSelected ? 'transform  scale-110 ' : ''} justify-center items-end m-2 rounded-md `}
            onClick={() => handleCardClick(id)}
        >
            <div className={`w-52 h-auto bg-white rounded-md text-center pt-3 mb-3 ${isSelected ? 'transform bg-green4 scale-105 h-28 transition-all ' : ''} `}>
                ...
                {isSelected ?
                    <div className='relative' style={{ marginTop: "-8.2rem", marginLeft: "-1rem" }}>
